      ...
      <div className="bg-slate-800 rounded-2xl w-full max-w-lg border border-slate-700 shadow-2xl animate-scale-in">
        ...
        <div className="p-6 space-y-4">
          ...
          {returnType === 'product' && (
            <>
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Товар (со склада) *</label>
                <select
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-amber-500 outline-none"
          ...
          {returnType === 'money' && (
            <>
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Сумма возврата *</label>
                <input
                  type="number"
                  value={moneyAmount}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">Способ возврата</label>
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() => setMoneyMethod('cash')}
                    className={`py-2 rounded-lg text-sm font-medium border transition-all ${moneyMethod === 'cash' ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-slate-900 border-slate-600 text-slate-400'}`}
                  >
                    Наличные
                  </button>
                  <button
                    onClick={() => setMoneyMethod('bank')}
                    className={`py-2 rounded-lg text-sm font-medium border transition-all ${moneyMethod === 'bank' ? 'bg-purple-500/20 border-purple-500 text-purple-400' : 'bg-slate-900 border-slate-600 text-slate-400'}`}
                  >
                    Р/С (Банк)
                  </button>
                  <button
                    onClick={() => setMoneyMethod('card')}
                    className={`py-2 rounded-lg text-sm font-medium border transition-all ${moneyMethod === 'card' ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'bg-slate-900 border-slate-600 text-slate-400'}`}
                  >
                    Карта
                  </button>
                </div>
                ...
                <div>
                  ...
                  <div className="flex bg-slate-900 rounded-xl p-1 border border-slate-600">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${moneyCurrency === 'UZS' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'}`}
                    >
                      Сум (UZS)
                    </button>
                    <button
                      onClick={() => setMoneyCurrency('USD')}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${moneyCurrency === 'USD' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'}`}
                    >
                      Доллар (USD)
                    </button>
                  </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Причина возврата</label>
                <input
                  type="text"
                  value={moneyReason}
              ...
              <button
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-3 rounded-xl font-bold text-lg shadow-lg shadow-emerald-600/20 transition-all mt-4"
              >
                Вернуть деньги
              </button>
            </>
