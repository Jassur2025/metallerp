      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className={`flex gap-1 ${t.bgCard} border ${t.border} rounded-lg p-1`}>
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${tab === 'create'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${tab === 'queue'
              ? theme === 'light' ? 'bg-[#1A73E8] text-white' : 'bg-slate-700 text-white'
              : t.textMuted + ' hover:' + t.text
              }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${tab === 'cancelled'
          ...
          <div ref={splitContainerRef} className="flex-1 flex min-h-0 gap-0">
            ...
            <div className={`${t.bgCard} border ${t.border} rounded-xl overflow-hidden flex flex-col min-h-0 min-w-0 ${t.shadow}`} style={{ width: `${100 - splitPercent}%`, maxHeight: 'calc(100vh - 160px)' }}>
              ...
              <div className={`p-3 space-y-2 flex-shrink-0 border-b ${t.border}`}>
                {/* Клиент - умный dropdown + быстрая кнопка добавления */}
                <div className="flex gap-1">
                  <div className="flex-1">
                    <ClientDropdown
                      ...
                      onAddClient={onSaveClients}
                      theme={theme}
                      t={t}
                    />
                  </div>
                  {/* Быстрое добавление нового мижоза */}
                  <button
                    type="button"
                    ...
                    className={`flex-shrink-0 flex items-center gap-1 px-2 py-1.5 rounded text-xs font-semibold transition-colors
                      ${theme === 'light'
              ...
              <div className={`p-3 border-t ${t.border} ${t.bgPanelAlt} space-y-1 flex-shrink-0`}>
                ...
                {editingOrder ? (
                  ...
                  <button
                    ...
                    disabled={!isSales}
                    className={`w-full mt-2 py-2 rounded-lg font-bold flex items-center justify-center gap-1 text-sm transition-colors ${!isSales
